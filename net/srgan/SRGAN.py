...
class Discriminator(nn.Module):
    def __init__(self,num_channel=3,base_filter=64):
        ...
        self.blocks = nn.Sequential(
            DiscriminatorBlock(base_filter,base_filter,kernel_size=3,stride=2,padding=1),
            DiscriminatorBlock(base_filter,base_filter*2,kernel_size=3,stride=1,padding=1),
            DiscriminatorBlock(base_filter*2,base_filter*2,kernel_size=3,stride=2,padding=1),
            DiscriminatorBlock(base_filter*2,base_filter*4,kernel_size=3,stride=1,padding=1),
            DiscriminatorBlock(base_filter*4,base_filter*4,kernel_size=3,stride=2,padding=1),
            DiscriminatorBlock(base_filter*4,base_filter*8,kernel_size=3,stride=1,padding=1),
            DiscriminatorBlock(base_filter*8,base_filter*8,kernel_size=3,stride=2,padding=1),
        )
        self.conv2 = nn.Conv2d(base_filter*8,num_channel,kernel_size=1,stride=1)
    def forward(self,x):
